...
import { useUpdateMissionsMutation } from "../../../slices/UpdateSlice";

export default function MissionsUpdate() {
    const { data, loading } = useGetMissionsQuery()
    const [newTitle, setNewTitle] = useState(data?.title ?? '')
    const [newText, setNewText] = useState(data?.text ?? '')
    const [updateMissionsPage, { isLoading, isError }] = useUpdateMissionsMutation()
  ...
  return (
    ...
    <div>
        <h2>modifier la page Missions</h2>

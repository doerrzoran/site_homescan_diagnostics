...
import { useUpdateElectricityMutation } from "../../../slices/UpdateSlice";

export default function ElectricityUpdate() {
    const { data, loading } = useGetElectricityQuery()
    const [newTitle, setNewTitle] = useState(data?.title ?? '')
    const [newText, setNewText] = useState(data?.text ?? '')
    const [updatePage, { isLoading, isError }] = useUpdateElectricityMutation()
  ...
  return (
    ...
    <div>
        <h2>modifier la page elecrticité</h2>
